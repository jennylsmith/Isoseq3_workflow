...
cat $dir/$fq | grep -E -A 1 "^@PB\.[0-9]" | grep -E -v "^--"| tr "@" ">" >$dir/${samp}.fasta

#create output directory
mkdir -p $dir/${samp}

#Run sqanti_qc.py
sqanti_qc.py -x $GMAPDB -t 8 -o $samp -d $dir/$samp -fl ${samp%.rep}.abundance.txt ${samp}.fasta $GTF $GENOME 2> ${samp}_sqantiQC.stderr

#create output directory
mkdir -p $dir/${samp}_filter

#run sqanti_filter.py
sqanti_filter.py -d $dir/${samp}_filter -i $dir/$samp/${samp}_corrected.fasta $dir/$samp/${samp}_classification.txt 2> ${samp}_sqantiFilt.stderr


# usig a GTF formatted file cell1-hq_transcripts.collapsed.gff
# sqanti_qc.py -g -o ${samp}_GTF -d $dir/${samp}_GTF \
# 		-fl ${samp%.rep}.abundance.txt ${samp%.rep}.gff $GTF $GENOME 2> ${samp}_sqantiQC_GTF.stderr &

#with Expression Data
# sqanti_qc.py -x $GMAPDB -t 8 -o $samp -d $dir/sqanti_results \
