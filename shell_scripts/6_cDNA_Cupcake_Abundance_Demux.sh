...
export PATH=~/.local/bin:$PATH #for minimap2 & cDNA_cupcake
...
#define samples
sample_idmap="~/scripts/Isoseq3_workflow/samples"
...
collapse_isoforms_by_sam.py --input ${hq_fq} --fq \
   -s ${hq_fq}.srt.sam -c 0.99 -i 0.95 --dun-merge-5-shorter -o ${prefix}_polished.hq.no5merge

#calculate abundance
get_abundance_post_collapse.py ${prefix}_polished.hq.no5merge.collapsed  ${prefix}_polished.cluster_report.csv

#Filter away 5' degraded isoforms
filter_away_subset.py ${prefix}_polished.hq.no5merge.collapsed
